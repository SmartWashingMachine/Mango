...
import numpy as np
from gandy.utils.knn_utils.generation_beam_constraints import Constraint, ConstraintListState
import copy
...
class BeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Old: self._done = torch.tensor([False for _ in range(batch_size)], dtype=torch.bool, device=self.device)
    ...
    def process(
        ...
        next_beam_scores = np.ndarray((batch_size, self.group_size), dtype=next_scores.dtype)
        next_beam_tokens = np.ndarray((batch_size, self.group_size), dtype=next_tokens.dtype)
        next_beam_indices = np.ndarray((batch_size, self.group_size), dtype=next_indices.dtype)
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id):
                    ...
                    beam_hyp.add(
                        # Old: input_ids[batch_beam_idx].clone(),
                        copy.deepcopy(input_ids[batch_beam_idx]),
                        next_score.item(),
    ...
    def finalize(
        ...
        # Old: sent_lengths = input_ids.new(batch_size * self.num_beam_hyps_to_keep)
        sent_lengths = np.zeros((batch_size * self.num_beam_hyps_to_keep), dtype=np.int64)

        best = []
        best_indices = []
        # Old: best_scores = torch.zeros(batch_size * self.num_beam_hyps_to_keep, device=self.device, dtype=torch.float32)
        best_scores = np.zeros((batch_size * self.num_beam_groups), dtype=np.float32)
        ...
        # Old: decoded: np.ndarray = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)
        decoded = np.zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len), dtype=np.int64)

        if len(best_indices) > 0 and best_indices[0] is not None:
            # Old: indices: np.ndarray = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)
            indices = np.zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len), dtype=np.int64)
        else:
            indices = None
        ...
        for i, (hypo, best_idx) in enumerate(zip(best, best_indices)):
            ...
            if indices is not None:
                # Old: indices[i, : len(best_idx)] = torch.tensor(best_idx)
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        next_tokens: np.ndarray,
        next_indices: np.ndarray,
        scores_for_all_vocab: np.ndarray,
        pad_token_id: Optional[int] = None,
        eos_token_id: Optional[int] = None,
    ) -> Tuple[np.ndarray]:
        r"""
        Args:
            input_ids (`np.ndarray` of shape `(batch_size * num_beams, sequence_length)`):
            ...
            next_scores (`np.ndarray` of shape `(batch_size, 2 * num_beams)`):
                Current scores of the top `2 * num_beams` non-finished beam hypotheses.
            next_tokens (`np.ndarray` of shape `(batch_size, 2 * num_beams)`):
                `input_ids` of the tokens corresponding to the top `2 * num_beams` non-finished beam hypotheses.
            next_indices (`np.ndarray` of shape `(batch_size, 2 * num_beams)`):
                Beam indices indicating to which beam hypothesis the `next_tokens` correspond.
            scores_for_all_vocab (`np.ndarray` of shape `(batch_size * num_beams, sequence_length)`):
        ...
        Return:
            `UserDict`: A dictionary composed of the fields as defined above:
                ...
                - **next_beam_scores** (`np.ndarray` of shape `(batch_size * num_beams)`) -- Updated scores of
                  all
                non-finished beams.

                - **next_beam_tokens** (`np.ndarray` of shape `(batch_size * num_beams)`) -- Next tokens to be
                  added
                to the non-finished beam_hypotheses.
                - **next_beam_indices** (`np.ndarray` of shape `(batch_size * num_beams)`) -- Beam indices
    ...
    def step_sentence_constraint(
        ...
        batch_idx: int,
        input_ids: np.ndarray,
        vocab_scores: np.ndarray,
        sent_beam_scores: np.ndarray,
        sent_beam_tokens: np.ndarray,
        sent_beam_indices: np.ndarray,
        ...
        # Old: full_hypotheses = torch.cat((input_ids[sent_beam_indices], sent_beam_tokens.unsqueeze(-1)), dim=-1)
        full_hypotheses = np.concatenate((input_ids[sent_beam_indices], sent_beam_tokens.unsqueeze(-1)), axis=-1)
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                advance_tokens = np.ndarray(advance_state.advance()).to(device)
                for advance_token in advance_tokens:
                    ...
                    # Old: advance_seq = torch.cat((pre_seq, advance_token.unsqueeze(0)), -1).cpu().tolist()
                    advance_seq = np.concatenate((pre_seq, advance_token.unsqueeze(0)), -1).tolist()
                ...
                # Old: new_score, new_token = torch.max(this_batch_token_scores[seq_idx], 0)  # some next probable token
                # Old: advance_seq = torch.cat((pre_seq, new_token.unsqueeze(0)), -1)
                new_token = np.argmax(this_batch_token_scores[seq_idx], 0)
                new_score = this_batch_token_scores[seq_idx][new_token]

                advance_seq = np.concatenate((pre_seq, new_token.unsqueeze(0)), -1)
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            """
            new_indices = np.array(track_new['new_indices'])
            new_tokens = np.stack(track_new['new_tokens'])
            new_scores = np.stack(track_new['new_scores'])
            ...
            """
            all_tokens = np.concatenate((sent_beam_tokens, new_tokens), -1)
            all_scores = np.concatenate((sent_beam_scores, new_scores), -1)
            all_banks = np.array([one.get_bank() for one in all_states])
            ...
            # Old: rearrangers = torch.tensor(np.argsort(increments, kind="mergesort"))
            rearrangers = np.argsort(increments, kind="mergesort")

            indices = indices[rearrangers][:orig_len]

            sent_beam_scores = all_scores[indices]
            sent_beam_tokens = all_tokens[indices]
            # Old: sent_beam_indices = torch.cat((sent_beam_indices, new_indices))[indices]
            sent_beam_indices = np.concatenate((sent_beam_indices, new_indices))[indices]
...
class BeamHypotheses:
    ...
    def add(self, hyp: np.ndarray, sum_logprobs: float, beam_indices: Optional[np.ndarray] = None):
        """
